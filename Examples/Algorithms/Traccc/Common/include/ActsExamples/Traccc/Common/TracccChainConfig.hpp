...
struct TracccChainConfig{
    ...
    using FittingConfigType = typename traccc::fitting_config<ScalarType>;
    using AmbiguityResolutionConfigType = typename traccc::greedy_ambiguity_resolution_algorithm::config_t;

    const SeedfinderConfigType seedfinderConfig;
    const SpacepointGridConfigType spacepointGridConfig{seedfinderConfig};
    const SeedfilterConfigType seedfilterConfig;
    const FindingConfigType findingConfig;
    const FittingConfigType fittingConfig;
    const AmbiguityResolutionConfigType ambiguityResolutionConfig;
};
