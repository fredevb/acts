...
def test_root_field_conversion():
    ...
    p = current_file_path.parent.parent.parent / "thirdparty" / "OpenDataDetector" / "data" / "odd-bfield.root"
    
    af = acts.examples.MagneticFieldMapXyz(str(p))
    cf = cc.covfieField(af)
    view = cc.newView(cf)
    points = [(9300.0, 4700.0, 11200.0),
    ...
    for x, y, z in points:
        ...
        assert (abs(Bx1 - Bx2) < error_margin_half_width and abs(By1 - By2) < error_margin_half_width and abs(Bz1 - Bz2) < error_margin_half_width)
